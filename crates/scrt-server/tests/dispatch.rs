...
fn palace_round_trip_via_dispatch() {
    ...
    let r = dispatch(
        "palace.drop",
        &json!({ "name": "s1", "palace_path": pal_s }),
    )
    .unwrap();
...
fn palace_similar_ranks_and_requires_a_query() {
    ...
    dispatch(
        "palace.stash",
        &json!({ "name": "auth", "note": "auth login rate limit throttle attempts",
        "palace_path": pal_s, "nodes": [mk("auth.rs", "fn check_auth_rate_limit() {}")] }),
    )
    .unwrap();
    dispatch(
        "palace.stash",
        &json!({ "name": "authz", "note": "auth login throttle rate limit guard",
        "palace_path": pal_s, "nodes": [mk("authz.rs", "fn check_auth_rate_limit() {}")] }),
    )
    .unwrap();
    dispatch(
        "palace.stash",
        &json!({ "name": "pizza", "note": "pizza dessert food recipes toppings cheese",
        "palace_path": pal_s, "nodes": [mk("food.md", "best pizza here")] }),
    )
    .unwrap();
    ...
    let r = dispatch(
        "palace.similar",
        &json!({ "name": "auth", "match": "full", "palace_path": pal_s }),
    )
    .unwrap();
    ...
    assert!(
        hits[0]["relevance"].as_f64().unwrap()
            > hits.last().unwrap()["relevance"].as_f64().unwrap()
    );
    ...
    let r = dispatch(
        "palace.similar",
        &json!({ "term": "auth login throttle", "palace_path": pal_s }),
    )
    .unwrap();
...
fn palace_compose_intersect_except_return_id_lists() {
    ...
    let comp = dispatch(
        "palace.compose",
        &json!({ "names": ["a", "b"], "palace_path": pal_s }),
    )
    .unwrap();
    ...
    let exc = dispatch(
        "palace.except",
        &json!({ "base": "a", "exclude": ["b"], "palace_path": pal_s }),
    )
    .unwrap();
...
fn palace_prune_keep_via_dispatch() {
    ...
    for n in ["a", "b", "c"] {
        dispatch(
            "palace.stash",
            &json!({ "name": n, "note": "", "palace_path": pal_s, "nodes": [node.clone()] }),
        )
        .unwrap();
    }
    let r = dispatch(
        "palace.prune_keep",
        &json!({ "n": 2, "palace_path": pal_s }),
    )
    .unwrap();
    ...
    assert_eq!(
        dispatch("palace.prune_keep", &json!({ "palace_path": pal_s }))
            .unwrap_err()
            .code
            .as_str(),
        "BAD_PARAMS"
